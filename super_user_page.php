...
$sql1="SELECT * FROM `agent_credentials`;";
...
<head>
    ...
    <link rel="stylesheet" href="/3_recruiters_portal/agent/styles/index.css">
    <link rel="stylesheet" href="/3_recruiters_portal/agent/styles/search.css">
    <link rel="stylesheet" href="/3_recruiters_portal/agent/styles/modal.css">
    ...
    <!-- <div class="tab-content" id="confirmed"> -->
            <?php
                if (isset($_POST["usubmit"]))
                {
                    ...
                    {
                        echo '<img src="/3_recruiters_portal\image\computer-1331579_640.webp" width="100" height="100" style="margin-left:650px;">';
                        if($result-> num_rows>0)
                        {
                            echo "<h1 style='margin-left:150px;'>User Data:</h1>";
                            echo"<table border='1' style='margin-left:150px;height:150px;width:1000px;'>";
                            echo "<tr><th> Userid </th><th> First name</th> <th> Last name</th> <th>Email</th></tr>";
                            while($row=$result->fetch_assoc())
                            {
                                echo "<tr><td>" . $row['Userid'] ."</td><td>" . $row['First name'] ."</td> <td>" . $row['Last name'] ."</td><td>" . $row['Email'] ."</td> <td> <form method='post' action='super_user_page.php'><input type='hidden' id='id' name='id' value=".$row['Email'].">"."<input type='submit' value='Delete_user' name='Delete' style='height:40px;width:100px;'></form></td></tr>";
                            }
                            echo"</table>";      
                        }
                        else{
